...
This command populates the knowledge graph with comprehensive JAC programming
concepts, relationships, and learning paths based on content from https://jac-lang.org/

Usage:
    python manage.py populate_knowledge_graph
    python manage.py populate_knowledge_graph --force
    python manage.py populate_knowledge_graph --check-only
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if existing_concepts > 0 and not options['force'] and not options['check_only']:
                self.stdout.write(
                    self.style.WARNING(
            ...
            if options['verbose'] and result["concepts"]:
                self.stdout.write(
            ...
            # Show learning paths
            if options['verbose'] and result["graphs"]:
                self.stdout.write(
